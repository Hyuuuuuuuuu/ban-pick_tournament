...
const PoolManager = ({ allSongs }) => {
  ...
  useEffect(() => {
    fetch(`${API_URL}/api/pools`).then(res => res.json()).then(data => { if(data) setPoolConfig(data); });
  }, []);
  ...
  const savePoolConfig = async () => { 
    await fetch(`${API_URL}/api/save-pools`, { 
        method: 'POST', 
  ...
  const handleUpdateSong = async () => {
      const res = await fetch(`${API_URL}/api/songs/update`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(editForm) });
  ...
  const getTypeLogo = (type) => {
      const imgPath = type === 'DX' ? `${API_URL}/assets/music_dx.png` : `${API_URL}/assets/music_standard.png`;
