...
const MatchControl = ({ socket, allSongs, roster, onRosterUpdate }) => {
  ...
  const addPlayer = async () => { if (!newPlayerName.trim()) return; const res = await fetch('http://localhost:3001/api/roster', { method: 'POST', headers: {'Content-Type': 'application/json'}, body: JSON.stringify({ name: newPlayerName }) }); const data = await res.json(); if (data.success) { onRosterUpdate(data.roster); setNewPlayerName(""); } };
  const removePlayer = async (name) => { if (window.confirm(`Xóa ${name}?`)) { const res = await fetch(`http://localhost:3001/api/roster/${name}`, { method: 'DELETE' }); const data = await res.json(); if (data.success) onRosterUpdate(data.roster); } };
  const updatePlayers = async () => { await fetch('http://localhost:3001/api/update-players', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ p1: p1Name, p2: p2Name }) }); };
  const setupRound = async (r) => { await updatePlayers(); setCurrentRound(r); await fetch(`http://localhost:3001/api/setup/${r}`); };
  const rerollRound = async () => { if (currentRound && window.confirm(`REROLL ${currentRound}?`)) await setupRound(currentRound); };
